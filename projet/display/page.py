...
class Page():
    ...
    def __call__(self, p):
        ...
        eval(f"self.page_{p}_setup()")
        epd.displayPartial(epd.getbuffer(self.image))
        self.page_change_flag = 1

        while self.page_change_flag:
            t = time.time()
            cmd = f"self.page_{p}_update()"
            # print(cmd)
            eval(cmd)
    ...
    def page_1_update(self):
        ...
        self.draw.text((0, 26), f'IP: {str(getIP())}', font=font(16), fill=self.background_color)
        ...
        self.draw.text((0, 88), f'RAM: {str(Ram_usage)}%', font=font(16), fill=255 - self.background_color)
    ...
    def page_2_update(self):
        ...
        self.draw.text((207, 75), f'{str(fan_power_read())}%', font=font(15), fill=255 - self.background_color)
